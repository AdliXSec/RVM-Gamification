...
// Fix the Machine Status location p tag manually because regex above missed it
...
content = content.replace(
  ...
  (match) => {
    return match
      ...
      .replace(/w-10 h-10/g, 'w-10 h-10 md:w-14 md:h-14') // Will conflict with empty state w-10 h-10, so be careful. 
      .replace(/mb-1/g, 'mb-1 md:mb-2')
...
// Fix empty state w-10 h-10 md:w-14 md:h-14 back to w-16 h-16
content = content.replace(/md:w-16 md:h-16 md:w-14 md:h-14/g, 'md:w-16 md:h-16');
...
content = content.replace(
  ...
  (match) => {
    return match
      ...
      .replace(/h-12 flex/g, 'h-12 md:h-16 flex')
      .replace(/mb-2"/g, 'mb-2 md:mb-4"')
      .replace(/text-base block pt-2/g, 'text-base md:text-xl block pt-2')
      .replace(/w-4 h-4/g, 'w-4 h-4 md:w-6 md:h-6')
      .replace(/mb-1"/g, 'mb-1 md:mb-2"')
      ...
      .replace(/h-16 flex/g, 'h-16 md:h-24 flex')
      .replace(/mt-1\.5/g, 'mt-1.5 md:mt-3')
      .replace(/text-base block"/g, 'text-base md:text-2xl block mt-1"')
      
      // Podium 3rd
      .replace(/-top-20/g, '-top-20 md:-top-32')
      .replace(/w-24 h-24/g, 'w-24 h-24 md:w-36 md:h-36')
      .replace(/h-10 flex/g, 'h-10 md:h-12 flex')
      .replace(/text-base block pt-1/g, 'text-base md:text-xl block pt-1 md:pt-2')
      ...
      // Text replacement conflicts fix
      .replace(/text-xs md:text-xl/g, 'text-xs md:text-lg') // Downgrade rank slightly
  }
);
