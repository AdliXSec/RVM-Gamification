    ...
    <div className="min-h-screen bg-[hsl(220,15%,7%)] text-slate-200 overflow-x-hidden selection:bg-green-500/30 font-pixel-body">
      ...
      <section className="relative min-h-screen flex items-center pt-20 pb-10 px-4 md:px-12 overflow-hidden scanlines">
        ...
        <div className="absolute inset-0 w-full h-full z-0 overflow-hidden">
          ...
          {/* Green gradient overlay to maintain the theme */}
          <div className="absolute inset-0 bg-gradient-to-b from-[hsl(220,15%,7%)]/80 via-green-950/40 to-[hsl(220,15%,7%)] mix-blend-multiply" />
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center mt-8">
          ...
          <div className="flex flex-col items-start text-left">
            ...
            <Link
              to="/register"
              className="pixel-btn bg-green-700 hover:bg-green-600 text-green-50 px-10 py-5 flex items-center gap-4 text-[12px] group relative overflow-hidden transition-all duration-300 shadow-[0_0_20px_rgba(34,197,94,0.4)] hover:shadow-[0_0_30px_rgba(34,197,94,0.6)]"
            >
      ...
      {/* Marquee Banner */}
      <div className="w-full bg-green-950 border-y-4 border-green-900 py-3 overflow-hidden flex relative z-10 shadow-[0_0_15px_rgba(34,197,94,0.2)]">
        <div className="flex animate-marquee whitespace-nowrap">
          {Array.from({length: 5}).map((_, i) => (
            <div key={i} className="flex items-center gap-8 mx-8">
              <span className="font-pixel text-[10px] text-green-400">+++ NEW REWARDS +++</span>
              <span className="font-pixel text-[10px] text-green-200">VOUCHER KANTIN</span>
              <span className="font-pixel text-[10px] text-green-200">MERCHANDISE EKSKLUSIF</span>
              <span className="font-pixel text-[10px] text-green-200">POTONGAN UKT</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <section className="bg-[hsl(220,18%,10%)] relative z-10 py-20 px-4 border-b-4 border-[hsl(220,15%,15%)]">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h2 className="font-pixel text-green-400 text-xl md:text-2xl drop-shadow-md">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              { icon: <Activity className="w-8 h-8 text-green-400" />, title: 'MONITORING LIVE', desc: 'Pantau kapasitas mesin secara real-time dari dashboard sebelum menyetor.' },
            ].map((f, i) => (
              <div key={i} className="pixel-border bg-[hsl(220,15%,12%)]/90 backdrop-blur-sm p-8 pixel-card group hover:bg-[hsl(220,20%,15%)] transition-colors border-2 border-[hsl(220,20%,20%)] hover:border-green-900/50">
                ...
                <h3 className="font-pixel text-slate-100 text-[11px] mb-4 group-hover:text-green-300 transition-colors">{f.title}</h3>
      ...
      <section className="bg-[hsl(220,15%,7%)] px-4 py-24 relative z-10">
        <div className="max-w-4xl mx-auto pixel-border bg-gradient-to-b from-[hsl(220,20%,12%)] to-[hsl(220,15%,8%)] p-8 md:p-12 text-center shadow-[0_0_40px_rgba(34,197,94,0.1)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4">
            {[
              ...
              { val: '616 kg', label: 'CO2 DIHINDARI', icon: <Shield className="w-5 h-5 mx-auto mb-3 text-green-500" /> },
              ...
              <div key={i} className="p-4 transform hover:scale-110 transition-transform duration-300">
                ...
                <div className="font-pixel text-[8px] text-green-600 tracking-wider">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer className="bg-[hsl(220,18%,5%)] px-4 py-8 text-center border-t-2 border-[hsl(220,15%,12%)] relative z-10">
        <p className="font-pixel text-[8px] text-slate-500 flex items-center justify-center gap-2">
          <Zap className="w-3 h-3 text-green-600" /> 2026 RVM QUEST - TELKOM UNIVERSITY SURABAYA
        </p>
      </footer>
    </div>
  );
