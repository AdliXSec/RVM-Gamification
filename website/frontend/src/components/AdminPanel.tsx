...
export default function AdminPanel() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[hsl(220,15%,7%)] scanlines overflow-x-hidden relative">
      ...
      {/* ═══ Header ═══ */}
      <header className="bg-slate-900/80 border-b-2 border-cyan-900/40 sticky top-0 z-20 backdrop-blur-lg">
      ...
      <div className="flex flex-1 relative z-10">
        {/* ═══ Sidebar (Desktop) ═══ */}
        <aside className="hidden md:flex flex-col w-56 bg-slate-900/60 border-r border-slate-800/60 backdrop-blur-sm sticky top-14 h-[calc(100vh-3.5rem)]">
        ...
        {/* ═══ Mobile Nav ═══ */}
        <div className="md:hidden bg-slate-900/60 border-b border-slate-800/60 px-2 py-2 flex gap-1 overflow-x-auto sticky top-14 z-10 backdrop-blur-sm">
          {navItems.map(n => (
            <button key={n.key} onClick={() => setTab(n.key as typeof tab)}
