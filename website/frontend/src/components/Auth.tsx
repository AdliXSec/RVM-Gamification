...
export default function Auth({ mode }: { mode: 'login' | 'register' }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-slate-950 p-4 py-20 scanlines relative">
      {/* Background Image Layer */}
      <div 
        className={`fixed inset-0 w-full h-full bg-cover md:bg-[length:100%_100%] bg-center pointer-events-none z-0 ${theme === 'dark' ? "bg-slate-900/80 bg-[url('/bg.jpeg')] bg-blend-multiply" : "bg-white/50 bg-[url('/bg-light.jpeg')] bg-blend-overlay"}`}
      />

      <Link to="/" className="fixed top-6 left-6 font-pixel text-[9px] text-slate-700 dark:text-slate-300 hover:text-green-600 dark:hover:text-green-400 flex items-center gap-2 z-10 bg-white/80 dark:bg-slate-900/50 p-2 rounded pixel-border backdrop-blur-sm">
        <ArrowLeft className="w-4 h-4" /> BACK
      </Link>
      
      <button 
        onClick={toggleTheme} 
        className="fixed top-6 right-6 p-2 pixel-border bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 hover:text-green-600 transition-colors z-20"
      >
      ...
      <div className="w-full max-w-sm space-y-6 z-10 my-8">
        <div className="text-center space-y-3">
          ...
          <h1 className="font-pixel text-slate-900 dark:text-slate-100 text-sm drop-shadow-md">RVM QUEST</h1>
          <p className="font-pixel-body text-slate-700 dark:text-slate-300 text-lg">
        ...
        {mode === 'login' ? (
          <>
            <div className="grid grid-cols-2 gap-1 p-1 bg-slate-200 dark:bg-slate-950 pixel-border border-slate-300 dark:border-slate-700">
              <button onClick={() => setTab('student')}
                className={`font-pixel text-[8px] py-2.5 transition-colors ${tab === 'student' ? 'bg-green-800 text-green-100' : 'text-slate-600 hover:text-slate-600 dark:text-slate-400'}`}>
                MAHASISWA
              </button>
              <button onClick={() => setTab('admin')}
                className={`font-pixel text-[8px] py-2.5 transition-colors ${tab === 'admin' ? 'bg-slate-700 text-slate-100' : 'text-slate-600 hover:text-slate-600 dark:text-slate-400'}`}>
                ADMIN
              </button>
            </div>

            <div className="pixel-border bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 p-6">
              {tab === 'student' ? (
                <form onSubmit={handleStudentLogin} className="space-y-5">
                  <div className="space-y-2">
                    <label className="font-pixel text-[8px] text-slate-500 dark:text-slate-400">EMAIL</label>
                ...
                <form onSubmit={handleAdmin} className="space-y-5">
                  <div className="space-y-2">
                    <label className="font-pixel text-[8px] text-slate-500 dark:text-slate-400">EMAIL ADMIN</label>
          ...
          <div className="pixel-border bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 p-6">
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="space-y-2">
                <label className="font-pixel text-[8px] text-slate-500 dark:text-slate-400">NAMA</label>
              ...
              <div className="space-y-2">
                <label className="font-pixel text-[8px] text-slate-500 dark:text-slate-400">NIM</label>
              ...
              <div className="space-y-2 pt-2 border-t-2 border-slate-200 dark:border-slate-700">
                <label className="font-pixel text-[8px] text-slate-500 dark:text-slate-400">PILIH AVATAR QUEST</label>
                <div className="grid grid-cols-3 gap-2">
                  {CHARACTERS.map(c => (
                    <button 
                      key={c} type="button" 
                      onClick={() => setSelectedChar(c)}
                      className={`p-2 pixel-border transition-colors ${selectedChar === c ? 'bg-green-900/40 border-green-500' : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border-slate-300 dark:border-slate-700'}`}
                    >
